...
- Add stable-target `eon tab close TAB` for non-final tabs. Pending tabs stop
  their picker before removal; durable tabs stop and prune each confirmed Orbit
  Session and retain every still-live mapping after a partial failure.
- Add semantic CLI actions that move the active tab left/right or selected pane
  ...
  stopping any Orbit Session. No native close or movement shortcut is assigned
  yet.
