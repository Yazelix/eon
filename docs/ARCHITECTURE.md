...
|---|---|---|---|
| Runtime and supervisor lifecycle | `crates/eon/src/main.rs` | CLI dispatch, launch mode, private configuration and product runtime roots, canonical Orbit identity validation and lease collection, deterministic same-boot recovery policy, supervisor-side presentation and stop, component launch policy, and the control socket | Generation discovery policy, PTYs, terminal state, native rendering, persistent topology, or managed-tool behavior |
