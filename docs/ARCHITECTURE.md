...
|---|---|---|---|
| Runtime and supervisor lifecycle | `crates/eon/src/main.rs` | CLI dispatch, launch mode, private configuration and product runtime roots, supervisor composition, presentation policy, and component selection | Generation discovery policy, EONW transport, Orbit Session lifecycle, PTYs, terminal state, native rendering, persistent topology, or managed-tool behavior |
