...
|---|---|---|---|
| CLI and executable boundary | `crates/eon/src/main.rs` | Invocation selection, CLI argument and output projection, managed-tool dispatch, and top-level error-to-exit mapping | Supervisor lifecycle, generation policy, EONW transport, workspace state, child mechanisms, or rendering |
